defmodule AIS.AisTest do
  use ExUnit.Case

  describe "decode sentences" do

    test "decode class 1" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,13IbQQ000100lq`LD7J6Vi<n88AM,0*52")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 3" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,39NSDjP02201T0HLBJDBv2GD02s1,0*14")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 4" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,402;bFQv@kkLc00Dl4LE52100@J6,0*58")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 5" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,2,1,9,B,53qH`N0286j=<p8b220ti`62222222222222221?9p;554oF0;B3k51CPEH888888888880,0*68")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 6" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,6>jCKIkfJjOt>db;q700@20,2*16")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 7" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,777QkG00RW38,0*62")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 8" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,83HT5APj2P00000001BQJ@2E0000,0*72")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 9" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,91b55vRAirOn<94M097lV@@20<6=,0*5D")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 10" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,:81:Jf1D02J0,0*0E")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 12" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,<42Lati0W:Ov=C7P6B?=Pjoihhjhqq0,2*2B")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 14" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,>5?Per18=HB1U:1@E=B0m<L,2*51")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 16 - 144 bit" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,@6STUk004lQ206bCKNOBAb6SJ@5s,0*74")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 17" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,A04757QAv0agH2JdGlLP7Oqa0@TGw9H170,4*5A")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 18" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,B3HOIj000H08MeW52k4F7wo5oP06,0*42")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 19" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,C69DqeP0Ar8;JH3R6<4O7wWPl@:62L>jcaQgh0000000?104222P,0*32")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 20" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,Dh3OvjB8IN>4,0*1D")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 21" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,E>jCfrv2`0c2h0W:0a2ah@@@@@@004WD>;2<H50hppN000,4*0A")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 22" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,F030p?j2N2P73FiiNesU3FR10000,0*32")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 23" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,B,G02:Kn01R`sn@291nj600000900,2*12")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 24 part B" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,H3HOIFTl00000006Gqjhm01p?650,0*4F")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 24 part A" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,H3HOIj0LhuE@tp0000000000000,2*2B")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
      assert attr.msg_type == 24
      assert attr.mmsi == "227006920"
    ...
    test "decode class 25" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,2,1,3,A,I`1ifG20UrcNTFE?UgLeo@Dk:o6G4hhI8;?vW2?El>Deju@c3Si451FJd9WPU<>B,0*04")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 26" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,J1@@0IK70PGgT740000000000@000?D0ih1e00006JlPC9C3,0*6B")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
    ...
    test "decode class 27" do
      {:ok, sentence} = AIS.Data.NMEA.parse("!AIVDM,1,1,,A,KCQ9r=hrFUnH7P00,0*41")
      {:ok, attr} = AIS.Data.Ais.parse(sentence.payload, sentence.padding)
